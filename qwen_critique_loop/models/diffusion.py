...
class DiffusionModel:
    ...
    def _ensure_loaded(self) -> None:
        ...
        # DPM++ multistep is faster and better at low step counts than Euler.
        self.pipe.scheduler = DPMSolverMultistepScheduler.from_config(
            self.pipe.scheduler.config
        )
        self.pipe = self.pipe.to("cpu")
        self.pipe.set_progress_bar_config(leave=False)
        # Memory savings on a 32GB box are nice-to-have; attention slicing
        # also gives a small CPU speedup at low resolution.
